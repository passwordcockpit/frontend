...
export default Component.extend(formValidation, {
  ...
  actions: {
    ...
    GeneratorPassword() {
      ...
      if (
        $('input[name="password-length"]').val() > 0 &&
        $('input[name="password-length"]').val() <= 100
      ) {
    ...
    deletePasswordFile() {
      ...
      $.ajax({
        ...
        .done(() => {
          ...
          let dataPassword = self
            .get('store')
            .peekRecord('password', self.password.get('id'));
          ...
          if (
            ...
            $.ajax({
              ...
              headers: {
                Authorization:
                  'Bearer ' +
                  self.get('session.session.content.authenticated.token'),
              },
              cache: false,
              contentType: false,
              processData: false,
              ...
              .done((fileCreatedResult) => {
                let passwordData = this.store.peekRecord(
                  'password',
                  fileCreatedResult.password_id,
                );
    ...
    setPassword(toggleFrontendCrypted) {
      if (toggleFrontendCrypted) {
        this.set(
          ...
          !this.get('password.frontendCrypted'),
        );
      }
      if (this.isEdit) {
        if (this.get('password.frontendCrypted')) {
          // with PIN
          let passwordEncrypted = this.passwordEncrypt.encryptPassword(
            this.pinEncrypt,
