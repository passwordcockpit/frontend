...
export default Component.extend(formValidation, {
  ...
  isActive: computed(
    ...
    'router.currentURL',
    function () {
      return this.router.isActive('folders.folder', this.folder.id);
    }
  ),
  scrollOnActive: observer('isActive', function () {
    if (this.isActive) {
      const scrollParam = this.get('router.currentRoute.queryParams.scroll');
      if (scrollParam) {
        scheduleOnce('afterRender', this, this.scrollToElement);
      }
    }
  }),
  scrollToElement() {
    const $container = $('.folder-menu.area .scroll-container');
