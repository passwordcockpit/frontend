...
export default Controller.extend({
  ...
  actions: {
    ...
    onDeleteFolder(folderId) {
      ...
      $.ajax({
        url:
          window.APP.host + '/' + window.APP.namespace + '/folders/' + folderId,
        method: 'DELETE',
        headers: {
        ...
        .done(() => {
          ...
          if (this.transitionData.to.attributes != undefined) {
            let transitionFolderId =
              this.transitionData.to.attributes['folderId'];
        ...
        .fail((adapterError) => {
          window.loading.hideLoading();
          this.growl.errorShowRaw(
            adapterError.responseJSON.title,
      ...
      return RSVP.hash(result)
        .then((hash) => {
          ...
          if (hash.target === 'Folder' || hash.target === 'All') {
            resultsSearch = this.buildSearchFoldersResult.call(
              this,
              hash.folders,
              resultsSearch
            );
          ...
          if (hash.target === 'Password' || hash.target === 'All') {
            resultsSearch = this.buildSearchPasswordsResult.call(
              this,
              hash.passwords,
              resultsSearch
            );
          ...
          resultsSearch = resultsSearch.sort(function (a, b) {
            let arrayLength =
              a.valueToSort.length < b.valueToSort.length
                ? a.valueToSort.length
                : b.valueToSort.length;
            for (let i = 0; i < arrayLength; i++) {
              if (
                a.valueToSort[i].toString().toLowerCase() <
                b.valueToSort[i].toString().toLowerCase()
              ) {
                return -1;
              } else if (
                b.valueToSort[i].toString().toLowerCase() <
    ...
    onSelectSearchPasswordElement(folderId, passwordId) {
      ...
      this.transitionToRoute(
        'folders.folder.passwords.password',
        folderId,
        passwordId
      );
    },
